...
public class SettingWiFi_type_Setup_ViewBinding implements Unbinder {
  private SettingWiFi_type_Setup target;

  private View view2131296670;

  @UiThread
  public SettingWiFi_type_Setup_ViewBinding(final SettingWiFi_type_Setup target, View source) {
    this.target = target;

    View view;
    target.edt_ip_address = Utils.findRequiredViewAsType(source, R.id.edt_ip_address, "field 'edt_ip_address'", EditText.class);
    target.edt_sub_mask = Utils.findRequiredViewAsType(source, R.id.edt_sub_mask, "field 'edt_sub_mask'", EditText.class);
  ...
  @CallSuper
  public void unbind() {
    SettingWiFi_type_Setup target = this.target;
    if (target == null) throw new IllegalStateException("Bindings already cleared.");
    this.target = null;

    target.edt_ip_address = null;
    target.edt_sub_mask = null;
